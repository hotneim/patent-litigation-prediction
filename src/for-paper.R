...
result_files <- dir("../rmd/objects", pattern = "*-results*")
...
for(i in 1:length(result_files)) {
  ...
  # Load the results
  load(paste0("../rmd/objects/", result_files[i]))
...
results$auc %>% 
  ...
  select(c(1,3,2)) %>% 
  kbl(digits = 3, booktabs = T, format = "latex", caption = "The AUC for the logistic regression.", label = "auc-lr", ) %>% 
  save_kable(file = "../../latex/material/revision_auc_logistic_regression.tex")
...
results$auc %>% 
  ...
  rename("Elstic net" = elastic,
         ...
         "xgBoost" = xgb) %>% 
  select(c(2,1,3,4)) %>% 
  kbl(digits = 3, booktabs = T, format = "latex", caption = "The AUC for the different models using blocks 1 to 5", label = "auc-all-models", ) %>% 
  save_kable(file = "../../latex/material/revision_auc_all_models.tex")
...
vi_aggregated$block <- factor(vi_aggregated$block, levels = c("1", "2", "3", 
                                                              "4", "5"))
vi_aggregated$cumulative <- factor(vi_aggregated$cumulative, levels = c("Using the blocks separately",
                                                            "Cumulative addition of blocks"))
vi_perm$cumulative <- factor(vi_perm$cumulative, levels = c("Using the blocks separately",
                                                                        "Cumulative addition of blocks"))
vi_aggregated$variable <- 
  factor(vi_aggregated$variable, levels = 
           vi_aggregated %>% 
           arrange(block) %>% 
...
ggsave(filename = "../../latex/material/revision_vi_logistic.pdf", plot = vi_logistic)

# --- PERMUTED VI PLOTS LOGISTIC REGRESSIOM BLOCK 1-2-3-4-5 ----------------------
variable_names <- read_csv("variable_names.csv")
...
vi_aggregated$variable <- fct_reorder(vi_aggregated$variable, vi_aggregated$mean_auc)
...
ggsave(filename = "../../latex/material/revision_vi_logistic_1_2_3_4_5.pdf", plot = vi_logistic_1_2_3_4_5)

# --- TABLE OF DESCRIPTIVE STATISTICS ------------------------------------------
load("../rmd/objects/block1-2-3-4-5-data.Rdata") 
variables <- read_excel("../../data/variables_revision.xlsx") %>% 
...
sumstats <- 
  ...
  mutate(small_entity_indicator_EX = as.numeric(litigation_df$small_entity_indicator_EX)-1) %>%
  ...
  mutate(., Max = ifelse(.$Variable == "nber_subcategory_PV", 
                          ...
                          no = .$Max)) %>% 
  mutate(Variable = update_name(Variable, variable_names)) %>% 
  kbl(digits = 1, booktabs = T, format = "latex", caption = "Summary statistics for the variables.", label = "sumstats") %>% 
  save_kable(file = "../../latex/material/revision_summary_statistics.tex")
...
ggsave(filename = "../../latex/material/revision_roc_all_models.pdf", 
       plot = roc_all_models, 
       width = 6, height = 6)
...
ggsave(filename = "../../latex/material/revision_roc_all_blocks.pdf", 
       plot = roc_all_blocks, 
       width = 6, height = 6)
...
ggsave(filename = "../../latex/material/revision_small_sample.pdf", plot = ss, width = 7, height = 5)

# --- TIME TO LITIGATIO FIGURE -------------------------------------------------
...
ggsave(filename = "../../latex/material/revision_time_to_litigation.pdf", 
       plot = litigation_lag)

# Make the same figure again, but only for patents that were granted in 2002.

litigation_lag_2002 <- 
  litigation_df %>% 
  select(grant_date_PV, litigation_filing_date) %>% 
  drop_na() %>% 
  filter(year(grant_date_PV) == 2002) %>% 
  mutate(lit_lag = litigation_filing_date - grant_date_PV) %>% 
  mutate(lit_lag = ifelse(lit_lag < 0, yes = 0, no = lit_lag)) %>% 
  ggplot(aes(x = lit_lag/365.25, y = ..density..)) +
  geom_histogram(binwidth = 1) +
...
ggsave(filename = "../../latex/material/revision_time_to_litigation_2002.pdf", 
       plot = litigation_lag_2002)
...
ggsave(filename = "../../latex/material/revision_odds_ratio.pdf", 
       plot = odds_ratio)


# CONFUSION MATRIX --------------
results$roc %>% filter(model == "xgb", where == "test") %>% 
...
predict(xgb_fit, new_data = litigation_test, type = "prob") %>% 
  ...
  mutate(predicted = ifelse(.pred_Litigated > 0.0136, yes = "Litigated", no = "Not litigated")) %>% 
